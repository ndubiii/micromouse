...
void test_start_to_search_transition()
{
    ...
    FakeSonar frontSonar(5); // obstacle close → triggers SEARCHING
...
// -------- TEST 2: TELEMETRY IS POPULATED --------
void test_telemetry_updates()
{
    FakeMotorDriver lmD, rmD;
    FakeEncoderDriver leD, reD;
    ...
    MouseTelemetry t = sm.getTelemetry();
...
// -------- TEST 3: SEARCHING LOOP STABLE --------
void test_searching_runs_stably()
{
    FakeMotorDriver lmD, rmD;
    FakeEncoderDriver leD, reD;
    FakeIR leftIR(200), rightIR(200);
...
// -------- TEST 4: SPEED FLAG BEHAVIOR --------
void test_speed_flag()
{
    FakeMotorDriver lmD, rmD;
    FakeEncoderDriver leD, reD;
    FakeIR leftIR(200), rightIR(200);
    ...
    // simulate many updates (eventually DECIDING/SPEED_RUN)
    for (int i = 0; i < 20; i++)
    {
        leD.ticks = 200;
        reD.ticks = 200;
    ...
    // If speed run started → flag must be true
...
void run_all_tests()
{
    ...
    RUN_TEST(test_telemetry_updates);
    RUN_TEST(test_searching_runs_stably);
    RUN_TEST(test_speed_flag);
